    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            for (var i = 0; i < results.length; i++){
                ...
                // submitblock and getinfo could absent in daemon implementation
                if ((rpcCall !== 'submitblock' && rpcCall !== 'getinfo') && (r.error || !r.result)){
            ...
            options.testnet = rpcResults.getinfo.testnet || (rpcResults.getmininginfo.chain !== "main" ? 1 : 0);
            options.protocolVersion = rpcResults.getinfo.protocolversion || rpcResults.getnetworkinfo.protocolversion;

            options.initStats = {
                connections: rpcResults.getinfo.connections || rpcResults.getnetworkinfo.connections,
                difficulty: (rpcResults.getinfo.difficulty || rpcResults.getmininginfo.difficulty)* algos[options.coin.algorithm].multiplier,
        ...
        _this.auxes[index].daemon.cmd('getauxblock', [],
            function(result){
                if (result.error){
                    ...
                    _this.auxes[index].rpcData.target = bignum.fromBuffer(util.uint256BufferFromHash(_this.auxes[index].rpcData._target, {endian: 'little', size: 32}));
